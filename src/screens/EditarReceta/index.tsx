...
export const EditarReceta = () => {
    ...
    const handleSubmitRecipe = async (ev: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const { data } = await axios.patch(`https://recetario-app-edmv.herokuapp.com/recipes/${params.id}`, newRecipe, options);
            toast.success("La receta se modifico de manera exitosa.");
            history.push('/')
        } catch (error) {
            toast.error(error.response.data.message);
        }
    ...
    const handleRemoveIngredient = (ingredient: any) => {
        const filteredIngredients = ingredients.filter((ingredientFiltered: any) => ingredientFiltered._id !== ingredient._id);
